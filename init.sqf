...
player addAction ["Setup Sandbag Bunker","functions\setupSandbags.sqf","",0,false,true,"",' (getdir cursorTarget - getdir player < 40) && ([cursorTarget] call ICE_fnc_getObjectCategory == "staticweapon") && (count (nearestObjects [player, ["ICE_emptySandbagsTimberStack"], 5]) > 0) '];//TFR Compatibility
