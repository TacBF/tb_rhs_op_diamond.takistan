class ICE
{
      ...
      class teamName
		...
		class scoring
		{
			class tickets
			{
				...
				bluFor = 50;// default for 120mins game = 240

				opFor = 3; //specify total caches regardless of default mission duration // default for 120mins game = 4

        ticketsPerCache = 45; // Tickets given to BLUFOR when a cache is destroyed
			};
		};
	};
  class respawn
  {
    ...
    class SRP
    {
      maxFriendlySiteDist = 1100;
    };
    class HO
    {
      minSpacingDist = 600;
      maxFriendlySiteDist = 4000;
    };
    class infantry
        {
            baseDuration = 60;
  ...
  class gameModes
  {
    ...
    class SAD
    {
	  ...
	  /*class contestableForwardBases
		{
				maxTicketLossRatePerHour = 60; // default
				maxIntelGatheringRatePerHour = 30; // default
		};
		*/
    };
  };
